...
class TestRunner:
    ...
    def carregar(self, fitxer):
        """
        Carrega els problemes des d'un fitxer de text per al CKY booleà.
    ...
    def carregar_prob(self, fitxer):
        """
        Carrega els problemes des d'un fitxer de text per al CKY probabilístic.
        Les transformacions han d'incloure probabilitats com a últim element float.
        La resposta correcta ha de ser un float (la probabilitat esperada) o None.
        :param fitxer: Ruta al fitxer de problemes.
        """
        self.problemes = []
        with open(fitxer, "r", encoding="utf-8") as f:
        ...
        for bloc in contingut.split("Problema "):
            ...
            try:
                gramatica = GramaticaProbabilistica(
                    simbols=camps["simbols"],
                    inici=camps["inici"],
                    transformacions=camps["transformacions"]
                )
    ...
    def executar_prob(self, tolerancia=1e-6):
        """
        Executa els problemes probabilístics carregats amb carregar_prob.
        Compara el resultat amb la resposta correcta dins d'una tolerància.
        :param tolerancia: Diferència màxima permesa entre resultat i esperat.
        """
        if not self.problemes:
            print("No hi ha problemes.")
            return
